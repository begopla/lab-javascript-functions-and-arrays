...
function avg(anyMixedArray) {
  ...
  totalCharacters+= anyMixedArray[i].length
  }
  if (anyMixedArray.length < 1) {
    return null;
  }
  var averageLength = totalCharacters/anyMixedArray.length
  return averageLength 
...
var maxResult =1;
var maxNumberInArray=[]

for(let i=0; i<anyMatrix.length; i++){

  for(let j=0; j<anyMatrix[i].length; j++){
    if (anyMatrix[i][j]> maxNumberInArray){
      maxNumberInArray[i]=anyMatrix[i][j]
    }

  }
  maxResult = maxResult* maxNumberInArray[i]
}
return maxResult
}
